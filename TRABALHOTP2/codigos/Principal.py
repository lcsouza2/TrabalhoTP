...
with open("TRABALHOTP2\\registros.json", "r") as f:
    ...
    for i in json.load(f):
        if i["Logado"] is True:
            ...
            #Cria o arquivo do usuário ou lê ele
            try: 
...
def carregar_imagem(caminho):
    ...
    img_carregada = ImageTk.PhotoImage(img)
    return img_carregada
...
def entry_valido(entry, placeholder):
    """
    Lógica:
        Em Python, strings vazias são conisderadas False, logo, se digitado não houver nada, 
        ou .strip() remova os espações e a sting for vazia, 
        ou digitado for igual ao texto placeholder: retornará False, senão, retornará True  
    """
    digitado = entry.get()
    return digitado and digitado.strip() and digitado != placeholder

def atualizar_tabela():
    for i in tabela.get_children(): #get_children retorna os valores na tabela
        tabela.delete(i)
...
def adicionar_evento():
    ...
    imagem_add_evento = carregar_imagem("TRABALHOTP2\\imagens\\frame_add_event.png")
    label_add_evento = tk.Label(
        frame_add_evento, 
        image=imagem_add_evento
    )
    label_add_evento.place(anchor="center", relheight=1, relwidth=1, relx=0.5, rely=0.5)
    
    #Mantém a imagem para evitar ela de ser excluída pelo garbage collector
    label_add_evento.image=imagem_add_evento
...
def excluir_evento():
    ...
    #Label para posicionar a imagem
    imagem_rem_evento = carregar_imagem("TRABALHOTP2\\imagens\\frame_rem_event.png")
    label_rem_evento = tk.Label(
        frame_rem_evento, 
        image=imagem_rem_evento
    )
    label_rem_evento.place(anchor="center", relheight=1, relwidth=1, relx=0.5, rely=0.5)
    
    #Mantém a imagem para evitar ela de ser excluída pelo garbage collector
    label_rem_evento.image=imagem_rem_evento
...
def alterar_evento():
    ...
    imagem_alt_evento = carregar_imagem("TRABALHOTP2\\imagens\\frame_alt_event.png")
    label_alt_evento = tk.Label(
        frame_alt_evento, 
        image=imagem_alt_evento
    )
    label_alt_evento.place(anchor="center", relheight=1, relwidth=1, relx=0.5, rely=0.5)
    
    #Mantém a imagem para evitar ela de ser excluída pelo garbage collector
    label_alt_evento.image=imagem_alt_evento
...
imagem_botoes=carregar_imagem("TRABALHOTP2\\imagens\\frame_botoes.png")
label_botoes = tk.Label(
    frame_botoes, 
    image=imagem_botoes, 
    highlightthickness=0
)
label_botoes.place(relheight=1, relwidth=1)
...
imagem_grade = carregar_imagem("TRABALHOTP2\\imagens\\frame_tabela.png")
label_grade = tk.Label(
    frame_grade, 
    image=imagem_grade, 
    highlightthickness=0
)
label_grade.place(relheight=1, relwidth=1)
