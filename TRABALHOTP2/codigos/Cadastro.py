...
def cadastrar(entry_nome, entry_email, entry_telefone, entry_senha):
    if not (entry_valido(entry_nome, placeholders[0]) and 
        ...
        for i in usuarios_no_json:
            if dados["Nome"] == i["Nome"]: #Em caso de dados repetidos
                duplicado=True
        ...
        if not duplicado:
            ...
            subprocess.run(["python", "TRABALHOTP2\\codigos\\Principal.py"]) #Roda o arquivo principal

def logar():
    janela_principal.destroy() 
    subprocess.run(["python", "TRABALHOTP2\\codigos\\Login.py"]) #Roda o arquivo de login
